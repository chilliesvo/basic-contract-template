...
/**
 * @dev The function should return the corresponding Forwarder address for a specific blockchain, such as Ethereum or Binance.
 * @return Forwarder addresses
 */
const getForwarderAddresses = async () => {
    //* Get network */
    const network = await ethers.provider.getNetwork();
    switch (network.chainId) {
        case 1:
            return process.env.MONKEY_STAKE_FORWARDER_ETHEREUM.split(",");
        case 137:
            return process.env.MONKEY_STAKE_FORWARDER_POLYGON.split(",");
        case 56:
            return process.env.MONKEY_STAKE_FORWARDER_BNB.split(",");
        case 5:
            return process.env.MONKEY_STAKE_FORWARDER_GOERLI.split(",");
        case 80001:
            return process.env.MONKEY_STAKE_FORWARDER_MUMBAI.split(",");
        case 97:
            return process.env.MONKEY_STAKE_FORWARDER_BNBT.split(",");
        case 31337:
            return process.env.MONKEY_STAKE_FORWARDER_GOERLI.split(",");
        default:
            throw "No corresponding Forwarder address found.";
    }
};

/**
 * @dev The function should return the corresponding TokenWrapper address for a specific blockchain, such as Ethereum or Binance.
 * @return Forwarder addresses
 */
const getNativeTokenWrapperAddress = async () => {
    //* Get network */
    const network = await ethers.provider.getNetwork();
    switch (network.chainId) {
        case 1:
            return process.env.NATIVE_TOKEN_WRAPPER_ETHEREUM;
        case 137:
            return process.env.NATIVE_TOKEN_WRAPPER_POLYGON;
        case 56:
            return process.env.NATIVE_TOKEN_WRAPPER_BNB;
        case 5:
            return process.env.NATIVE_TOKEN_WRAPPER_GOERLI;
        case 80001:
            return process.env.NATIVE_TOKEN_WRAPPER_MUMBAI;
        case 97:
            return process.env.NATIVE_TOKEN_WRAPPER_BNBT;
        case 31337:
            return process.env.NATIVE_TOKEN_WRAPPER_GOERLI;
        default:
            throw "No corresponding NativeTokenWrapper address found.";
    }
};
